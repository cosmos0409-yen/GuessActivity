...
- **決賽題目一出現就自動開始計時（2026-09-17，使用者要求「不需要開始按鈕」）**：`App.tsx` 在「題目出現／換題」的 effect（`phase === "questionShown"` 時重設倒數）最後直接呼叫 `handleStart()`（dispatch `START`＋`mainCountdown.start()`＋倒數背景節拍）；主持人列拿掉「開始」鈕，`gameMachine` 的 `questionShown → START → counting` 流程與測試保留不動（UNDO 回到 questionShown 也會自動再開始）。空白鍵說明改成「暫停／恢復倒數」（`hotkeys.ts`、根目錄 README）。測試：`App.countdown.test.tsx`、`App.timeUp.test.tsx` 改成在點題型前裝假時鐘、不再按開始，並驗證沒有「開始」鈕、倒數自動遞減、時間到提示；`App.poll.test.tsx` 改等選項出現。原本「問題 3：還沒按開始時顯示完整秒數待命」的需求已被取代。tsc 0、vitest 211 過；dev 實測選題後 4 秒倒數 30 → 26、主持人列無「開始」。
