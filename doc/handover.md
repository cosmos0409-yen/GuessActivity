...
- **2026-09-14 更新**：100 題新題目獨立查核通過（`docs/新題查核報告.md`：99 題 PASS，K72 維持待審，沒有事實錯誤），**已經合併進 `public/sample-questions.csv`**：共 152 題，上架 149、待審 3（K14、K15、K72）；只有「數字的秘密」難度 3 是 2 題，其他每格都是 3 題。貼進使用者 Google 試算表的步驟要等 Chrome 視窗在前景（`navigator.clipboard.writeText` 在視窗沒有焦點時會卡住）。
